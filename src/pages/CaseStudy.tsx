...
const CaseStudy = () => {
  ...
  return (
    <main className="pt-24" onClick={handleMainClick}>
        ...
        <div className="mx-auto max-w-5xl">
            ...
            <ScrollReveal>
                  <div>
                    ...
                    <div className="grid gap-12 md:grid-cols-2 items-start">
                      <div className="overflow-hidden rounded-xl shadow-md">
                        <img src="https://raw.githubusercontent.com/DaithiOCheallaigh/lacuna/main/public/images/Pill6-1.webp" alt="White Label UX & Consumer Psychology Report" className="w-full object-cover" loading="lazy" />
                      </div>
                      <div>
            ...
            <ScrollReveal>
              ...
              {/* Detailed Design Process Phases 01-05 (before Key Features) */}
              {project.designProcessDetails &&
            project.designProcessDetails.filter((_, i) => i < project.designProcessDetails!.length - 1).map((phase, i) =>
      ...
      {/* Last Design Process Phase (after Key Features) */}
      {project.designProcessDetails && project.designProcessDetails.length > 0 &&
      (() => {
        const lastPhase = project.designProcessDetails[project.designProcessDetails.length - 1];
        return (
          <section className="px-6 py-12 md:px-12 lg:px-24">
            <div className="mx-auto max-w-5xl">
              <ScrollReveal>
                <div>
                  <SectionLabel>{`${lastPhase.num}/ ${lastPhase.title}`}</SectionLabel>
                  <div className="space-y-4 mb-6">
                    {lastPhase.text.split("\n").filter(Boolean).map((line, j) => {
                      if (line.startsWith("- ")) {
                        return (
                          <li key={j} className="ml-4 flex items-start gap-2 text-sm leading-relaxed text-muted-foreground">
                            <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-primary" />
                  ...
                  {lastPhase.outcomes &&
                    <div className="rounded-xl bg-card p-6 mb-6">
                      <h4 className="mb-2 text-sm font-bold text-foreground">Outcomes</h4>
                      <p className="text-sm text-muted-foreground">{lastPhase.outcomes}</p>
                    </div>
                  }
                  {lastPhase.image &&
                    <div className="overflow-hidden rounded-xl shadow-md">
                      <img src={lastPhase.image} alt={lastPhase.title} className="w-full object-cover" loading="lazy" />
                    </div>
                  }
                </div>
              </ScrollReveal>
